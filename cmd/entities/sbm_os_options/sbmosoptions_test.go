...
func TestGetSBMOperatingSystemOptionCmd(t *testing.T) {
	testCases := []struct {
		name           string
		id             int64
		output         string
	...
	}{
		{
			...
			id:             testOSOptionID,
			flags:          []string{"--location-id", fmt.Sprint(testLocationID), "--model-id", fmt.Sprint(testServerModelID)},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(tc.flags) > 0 {
				locationsServiceHandler.EXPECT().
					GetSBMOperatingSystemOption(gomock.Any(), testLocationID, testServerModelID, tc.id).
					Return(&testOSOption, err)
			}
			testCmdContext := testutils.NewTestCmdContext(scClient)
			osCmd := NewCmd(testCmdContext)
			args := []string{"sbm-os-options", "get", fmt.Sprint(tc.id)}
			if len(tc.flags) > 0 {
...
func TestListSBMOperatingSystemOptionsCmd(t *testing.T) {
	o1 := testOSOption
	o2 := testOSOption
	...
	testCases := []struct {
	...
	}{
		{
			...
			output:         "json",
			args:           []string{"-A", "--location-id", "1", "--model-id", "100"},
		...
		},
		{
			...
			output:         "json",
			args:           []string{"--location-id", "1", "--model-id", "100"},
		...
		},
		{
			...
			args:           []string{"--page-view", "--location-id", "1", "--model-id", "100"},
	...
	locationsServiceHandler.EXPECT().
		SBMOperatingSystemOptions(gomock.Any(), gomock.Any()).
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			testCmdContext := testutils.NewTestCmdContext(scClient)
			osCmd := NewCmd(testCmdContext)
			args := []string{"sbm-os-options", "list"}
			if len(tc.args) > 0 {
